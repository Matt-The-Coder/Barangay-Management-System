...
<body>
                ...
                <div class="panel-header">
                    <div class="page-inner">
                        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                            <div>
                                <h2 class=" fw-bold">Barangay Officials</h2>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card">
                                    ...
                                    <div class="card-head-row">
                                        ...
                                        <?php if (isset($_SESSION['username'])) : ?>
                                            <div class="card-tools">
                                                <a href="#add" data-bs-toggle="modal" class="btn btn-info float-end btn-sm text-white">
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table nowrap table-borderless table-hover table-sm align-middle mb-0 bg-white" style="width:100%" id="tblofficial">
                                                ...
                                                <?php if (!empty($official)) : ?>
                                                    <?php foreach ($official as $row) : ?>
                                                        <tr>
                                                            ...
                                                            <?php if (isset($_SESSION['username'])) : ?>
                                                                <?php if ($_SESSION['role'] == 'administrator') : ?>
                                                                    <td><?= $row['status'] == 'Active' ? '<span class="badge rounded-pill badge-success text-success">Active</span>' : '<span class="badge rounded-pill badge-danger text-danger">Inactive</span>' ?>
                                                                    </td>
                                                                <?php endif ?>
                                                                <td>
                                                                    <a type="button" href="#edit" data-bs-toggle="modal" class="btn btn-warning text-white btn-sm" title="Edit Position" onclick="editOfficial(this)" data-id="<?= $row['id'] ?>" data-name="<?= $row['name'] ?>" data-chair="<?= $row['chair_id'] ?>" data-pos="<?= $row['pos_id'] ?>" data-start="<?= $row['termstart'] ?>" data-end="<?= $row['termend'] ?>" data-status="<?= $row['status'] ?>">
                                                                        <i class="ri-edit-2-line" style="font-size: 1rem;"></i>
                                                                    </a>
                                                                    <?php if ($_SESSION['role'] == 'administrator') : ?>
                                                                        <a type="button" data-bs-toggle="tooltip" href="model/remove_official.php?id=<?= $row['id'] ?>" onclick="return confirm('Are you sure you want to delete this official?');" class="btn btn-danger btn-sm text-white" data-original-title="Remove">
    ...
    <script>
        $(document).ready(function() {
            var oTable = $('#tblofficial').DataTable({
                "order": [
                    [3, "asc"]
                ]
            });
